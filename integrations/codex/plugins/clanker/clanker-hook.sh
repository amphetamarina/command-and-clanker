...
# ...); Grok reuses the Claude hook and is detected via its own env var. Only
# acts inside a Command & Clanker terminal (CLANKER_SESSION is injected there),
# and is fast and silent so it never blocks the agent — safe to install globally.
[ -z "$CLANKER_SESSION" ] && exit 0
[ -z "$CLANKER_INGEST" ] && exit 0
tool="${1:-claude}"
[ -n "$GROK_SESSION_ID" ] && tool="grok"
